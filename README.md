...
经过笔者干中学的实践理论是：告诉你香蕉沾辣椒才知道有这个地狱美食的食谱，同理函数的使用，如果没有对应高级函数的思就就不知道还有这种用法。
所以为了更好更有逻辑的快速总览与查找DolphinDB的全部函数。本人依照已发布的文档信息（文档侧边栏全部函数页面与DolphinDB函数参考\函数分类的页面）进行对照。补齐函数分类页面缺失但已发布的函数内容，并对页面进行重构。方便使用者快速的定位目标函数。
